...
export default function HistorialView() {
  ...
  const filtered = useMemo(() => {
    if (!search) return db.historial;
    const q = search.toLowerCase();
    return db.historial.filter(h => h.codigo.toLowerCase().includes(q) || h.cliente.toLowerCase().includes(q));
  }, [db.historial, search]);
  ...
  return (
    <div style={{ padding:28 }}>
      ...
      {filtered.length === 0 ? <EmptyState icon="archive" title="Sin registros" subtitle="Las entregas aparecerán aquí" /> : (
        <div style={{ background:'var(--bg-2)', border:'1px solid var(--border-1)', borderRadius:'var(--r3)', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr>
              {['Código','Cliente','Descripción','Prioridad','Creado','Entregado','Entregó'].map(h => (
              ...
              {filtered.map((h,i) => {
                ...
                return (
                  <tr key={h.id} className={'anim-fade d'+Math.min(i+1,8)} style={{ borderBottom: i<filtered.length-1?'1px solid var(--border-1)':'none' }}>
                    ...
                    <td style={{ padding:'11px 16px', fontSize:12, color:'var(--text-3)', maxWidth:220, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{h.descripcion}</td>
                    ...
                    <td style={{ padding:'11px 16px', whiteSpace:'nowrap' }}>
                      <span style={{ fontSize:12, color:'var(--success)', background:'#30A46C12', padding:'3px 8px', borderRadius:4 }}>{formatDate(h.fecha_entrega)}</span></td>

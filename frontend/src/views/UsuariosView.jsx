...
export default function UsuariosView() {
  ...
  const createUser = () => {
    ...
    if (db.usuarios.some((u) => u.username === form.username)) {
      showToast("Username ya existe", "error");
      return;
    }
    setDb((prev) => ({
      ...prev,
    ...
    addLog(
      session.user.nombre,
      "Creó usuario",
      form.username + " (" + ROLE_META[form.rol].label + ")",
      "usuario",
    );
    showToast("Usuario " + form.username + " creado");
    setShowCreate(false);
  ...
  const toggleUser = (u) => {
    setDb((prev) => ({
      ...prev,
      usuarios: prev.usuarios.map((usr) =>
        usr.id === u.id ? { ...usr, activo: !usr.activo } : usr,
      ),
    }));
    addLog(
      session.user.nombre,
      u.activo ? "Desactivó usuario" : "Activó usuario",
      u.username,
      "usuario",
    );
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    <Badge color={meta.color} style={{ fontSize: 10 }}>
                      {meta.label}
                    </Badge>
                  </div>
                </div>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ...
            <Btn variant="primary" icon="plus" onClick={createUser}>
              Crear
            </Btn>
          </div>
        </div>

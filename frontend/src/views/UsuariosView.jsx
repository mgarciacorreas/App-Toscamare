...
export default function UsuariosView() {
  const { usuarios, showToast, loadUsuarios } = useContext(AppContext);
        ...
        {usuarios.map((u, i) => {
          ...
          return (
            <div key={u.id} className={'anim-fade d' + Math.min(i + 1, 8)} style={{ background: 'var(--bg-2)', border: '1px solid var(--border-1)',
              borderRadius: 'var(--r3)', padding: '18px 20px', transition: '.15s var(--ease)' }}>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Crear Usuario">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--text-4)' }}>
            El usuario iniciará sesión con su cuenta de Microsoft asociada a este email.
          </p>

...
export default function DashboardView({ setView }) {
  ...
  const stats = Object.entries(ESTADOS).map(([e, cfg]) => ({
    estado: parseInt(e),
    ...cfg,
    count: db.pedidos_activos.filter((p) => p.estado_actual === parseInt(e))
      .length,
  }));
  const urgentes = db.pedidos_activos.filter(
    (p) => p.prioridad === "urgente",
  ).length;
  const myPedidos = isAdmin
    ? db.pedidos_activos
    : db.pedidos_activos.filter(
        (p) => p.estado_actual === ROLE_META[session.user.rol].estadoVisible,
      );
    ...
    <div style={{ padding: 28 }}>
      ...
      {urgentes > 0 && (
        ...
        >
          ...
          <span
            style={{ fontSize: 13, color: "var(--danger)", fontWeight: 500 }}
          >
            {urgentes} pedido{urgentes > 1 ? "s" : ""}{" "}
            <strong>URGENTE{urgentes > 1 ? "S" : ""}</strong>
          </span>
        </div>
      )}
          ...
          <div style={{ maxHeight: 320, overflowY: "auto" }}>
            {myPedidos.slice(0, 6).map((p) => (
              ...
              >
                ...
                <Badge color={ESTADOS[p.estado_actual].color}>
                  {ESTADOS[p.estado_actual].label}
                </Badge>
              </div>
            ))}
            {myPedidos.length === 0 && (
          ...
          <div style={{ maxHeight: 320, overflowY: "auto" }}>
            {db.log_actividad.slice(0, 8).map((log) => (
              ...
              >
                ...
                <div>
                  <p style={{ fontSize: 12 }}>
                    <span style={{ fontWeight: 500 }}>{log.usuario}</span>
                  ...
                  >
                    {log.detalle} · {timeAgo(log.timestamp)}
                  </p>
                </div>
              </div>
            ))}

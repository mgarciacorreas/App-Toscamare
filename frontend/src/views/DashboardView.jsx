...
export default function DashboardView({ setView }) {
  const { pedidos, historial, logActividad, session, loadHistorial, loadLog } = useContext(AppContext);
  const isAdmin = session.user.rol === ROLES.ADMIN;

  useEffect(() => {
    loadHistorial();
    if (isAdmin) loadLog();
  }, [loadHistorial, loadLog, isAdmin]);
    ...
    <div style={{ padding:28 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:14, marginBottom:28 }}>
        ...
        <div className="anim-fade d5" style={{ background:'var(--bg-2)', border:'1px solid var(--border-1)', borderRadius:'var(--r3)', padding:'18px 20px' }}>
          ...
          <span style={{ fontSize:34, fontWeight:700, letterSpacing:'-0.03em' }}>{historial.length}</span>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <div className="anim-fade d5" style={{ background:'var(--bg-2)', border:'1px solid var(--border-1)', borderRadius:'var(--r3)', overflow:'hidden' }}>
          ...
          <div style={{ maxHeight:320, overflowY:'auto' }}>
            {logActividad.slice(0,8).map(log => (
            ...
            {logActividad.length === 0 && <div style={{ padding:'30px 18px', textAlign:'center', color:'var(--text-4)', fontSize:13 }}>Sin actividad</div>}
          </div>
        </div>
      </div>
    </div>

...
const VIEW_META = {
  ...
  historial: ()  => ({ title:'Historial de Entregas', subtitle:'Pedidos completados y archivados' }),
  usuarios:  ()  => ({ title:'Gestión de Usuarios', subtitle:'Administrar cuentas y roles' }),
  actividad: ()  => ({ title:'Log de Actividad', subtitle:'Registro de acciones del sistema' }),
};
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:'var(--bg-0)' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        ...
        <main style={{ flex:1, overflowY:'auto' }}>
          ...
          {view === 'actividad' && session.user.rol === ROLES.ADMIN && <ActividadView />}
        </main>
      </div>
    </div>
  );

...
export default function ProductosTable({ pedidoId, editable, canManage }) {
  ...
  const saveEdit = async (prod) => {
    try {
      ...
      setProductos(prev => prev.map(p => p.id === prod.id ? { ...p, cantidad: Number(editVal) } : p));
      setEditingId(null);
  ...
  const handleAdd = async () => {
    if (!newNombre.trim() || !newCantidad) { showToast('Completa nombre y cantidad', 'error'); return; }
    try {
      const result = await api.addProducto(pedidoId, newNombre.trim(), Number(newCantidad));
  ...
  const inputStyle = { width: '100%', padding: '6px 8px', background: 'var(--bg-2)', border: '1px solid var(--border-2)', borderRadius: 4, color: 'var(--text-1)', fontSize: 13 };

  if (productos.length === 0 && !adding) {
    return (
      <div>
        <p style={{ fontSize: 12, color: 'var(--text-4)', marginBottom: 8 }}>Sin productos</p>
        {canManage && <Btn variant="secondary" size="sm" icon="plus" onClick={() => setAdding(true)}>Añadir producto</Btn>}
      </div>
    );
  }
    ...
    <div>
      <div style={{ background: 'var(--bg-1)', border: '1px solid var(--border-1)', borderRadius: 'var(--r2)', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            {['Producto', 'Cantidad (kg)', ...((editable || canManage) ? [''] : [])].map(h => (
          ...
          <tbody>
            {productos.map((p, i) => (
              <tr key={p.id} style={{ borderBottom: i < productos.length - 1 ? '1px solid var(--border-1)' : 'none' }}>
                <td style={{ padding: '8px 12px', fontSize: 13, fontWeight: 500 }}>{p.nombre_producto}</td>
                <td style={{ padding: '8px 12px', fontSize: 13 }}>
                  {editingId === p.id ? (
                    <input type="number" value={editVal} onChange={e => setEditVal(e.target.value)} min="0" step="0.01" autoFocus
                      style={{ ...inputStyle, width: 80, borderColor: 'var(--accent)' }}
                ...
                {(editable || canManage) && (
                  <td style={{ padding: '8px 12px', width: 100 }}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      {canManage && <Btn variant="ghost" size="sm" icon="trash" danger onClick={() => handleDelete(p)} />}
                    </div>
                  </td>
                )}
              </tr>
            ...
            {adding && (
              <tr style={{ borderTop: '1px solid var(--border-1)' }}>
                <td style={{ padding: '6px 8px' }}>
                  <input value={newNombre} onChange={e => setNewNombre(e.target.value)} placeholder="Nombre del producto"
                    style={inputStyle} autoFocus />
                </td>
                <td style={{ padding: '6px 8px' }}>
                  <input type="number" value={newCantidad} onChange={e => setNewCantidad(e.target.value)} placeholder="0"
                    min="0" step="0.01" style={{ ...inputStyle, width: 80 }}
                    onKeyDown={e => { if (e.key === 'Enter') handleAdd(); }} />
                </td>
                <td style={{ padding: '6px 8px' }}>
                  <div style={{ display: 'flex', gap: 4 }}>
                    <Btn variant="primary" size="sm" icon="check" onClick={handleAdd} />
                    <Btn variant="ghost" size="sm" icon="x" onClick={() => { setAdding(false); setNewNombre(''); setNewCantidad(''); }} />
                  </div>
                </td>
              </tr>
            )}
      ...
      {canManage && !adding && (
        <Btn variant="secondary" size="sm" icon="plus" onClick={() => setAdding(true)} style={{ marginTop: 8 }}>Añadir producto</Btn>
      )}
    </div>
  );
}

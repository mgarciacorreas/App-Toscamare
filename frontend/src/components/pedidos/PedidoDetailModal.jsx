...
export default function PedidoDetailModal({ open, onClose, pedido, onRefresh }) {
  ...
  const canManageProducts = [ROLES.OFICINA, ROLES.ALMACEN, ROLES.LOGISTICA, ROLES.ADMIN].includes(session.user.rol);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div>
          ...
          <div style={{ marginBottom: 14 }}>
            <span style={{ fontSize: 11, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Cliente</span>
            <p style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-1)', marginTop: 2 }}>{pedido.cliente}</p>
          </div>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <ProductosTable pedidoId={pedido.id} editable={canEditProducts} canManage={canManageProducts} />
          </div>
        </div>
          ...
          <div style={{ padding: 16, background: 'var(--bg-1)', borderRadius: 'var(--r2)', border: '1px solid var(--border-1)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
              {Object.entries(ESTADOS).map(([k, v]) => (
                <span key={k} style={{ fontSize: 9, color: pedido.estado_actual >= parseInt(k) ? v.color : 'var(--text-4)', textAlign: 'center', flex: 1 }}>
